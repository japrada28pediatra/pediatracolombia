  ...
  <<~HTML
    ...
    <body class="bg-offwhite text-navy font-sans antialiased">
      ...
      <main>
        ...
        <section class="px-4 pb-16"><div class="max-w-4xl mx-auto rounded-2xl border border-teal/20 bg-white p-6"><p class="font-bold mb-2">Fuente para ampliar</p><a href="#{esc(service[:source_url])}" target="_blank" rel="noopener noreferrer" class="text-teal underline">#{esc(service[:source_name])}</a><p class="mt-4 text-xs text-slate-500">Contenido informativo actualizado el <time datetime="#{UPDATED}">13 de julio de 2026</time>. No sustituye una valoración médica individual.</p></div></section>
